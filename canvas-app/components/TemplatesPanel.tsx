...
const TemplatesPanel: React.FC<TemplatesPanelProps> = ({
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div 
        className={`fixed top-0 right-0 h-full w-[480px] bg-canvas-card shadow-2xl z-50 transform transition-transform duration-300 ease-out flex flex-col ${
          isOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        {/* Header */}
        <div className="p-6 border-b border-cyan-900/30 bg-gradient-to-r from-cyan-900/20 to-emerald-900/20">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-xl font-bold text-gray-200">Templates</h2>
              <p className="text-sm text-canvas-muted-deep mt-1">Choose a template to get started</p>
            </div>
            <button
              onClick={onClose}
              className="p-2 hover:bg-cyan-500/10 rounded-xl transition-colors"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-canvas-muted-deep" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="relative">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 absolute left-3 top-1/2 -translate-y-1/2 text-canvas-muted-deep" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-canvas-card border border-cyan-900/30 rounded-xl text-sm text-canvas-text placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
            />
          </div>
        </div>

        {/* Language Filter */}
        <div className="px-6 py-4 border-b border-cyan-900/30 overflow-x-auto">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all ${
                selectedLanguage === 'all'
                  ...
                  : 'bg-canvas-card text-canvas-muted hover:bg-cyan-500/10 border border-cyan-900/30'
              }`}
            >
              All Languages
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
          {filteredTemplates.length === 0 ? (
            <div className="text-center py-12 text-canvas-muted-deep">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 mx-auto mb-3 opacity-50 text-cyan-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            filteredTemplates.map((template) => (
              <div
                key={template.id}
                className={`group relative bg-canvas-card border border-cyan-900/30 rounded-2xl p-4 hover:border-cyan-500/30 hover:shadow-lg transition-all duration-200 ${
                  hoveredTemplate === template.id ? 'ring-2 ring-cyan-500 ring-opacity-50' : ''
                }`}
              ...
              >
                ...
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h3 className="font-semibold text-gray-200 text-sm">{template.name}</h3>
                ...
                {/* Description */}
                <p className="text-xs text-canvas-muted mb-3 line-clamp-2">{template.description}</p>
                ...
                {template.tags && (
                  <div className="flex flex-wrap gap-1 mb-3">
                    {template.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2 py-0.5 bg-cyan-900/20 text-canvas-muted rounded text-[10px]"
                      >
                        #{tag}
                      </span>
                    ))}
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-gradient-to-r from-cyan-500 to-emerald-500 hover:from-cyan-600 hover:to-emerald-600 text-white text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-2 shadow-md hover:shadow-lg active:scale-[0.98]"
                >
        ...
        {/* Footer */}
        <div className="p-4 border-t border-cyan-900/30 bg-canvas-card">
          <p className="text-xs text-canvas-muted-deep text-center">
