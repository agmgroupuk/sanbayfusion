...
const fileIconMap: Record<string, { icon: React.ElementType; color: string }> = {
  ...
  css: { icon: FileCode, color: 'text-primary-400' },
  scss: { icon: FileCode, color: 'text-pink-400' },
  json: { icon: FileJson, color: 'text-amber-400' },
  md: { icon: FileText, color: 'text-canvas-muted' },
  svg: { icon: Image, color: 'text-emerald-400' },
  png: { icon: Image, color: 'text-emerald-400' },
  config: { icon: Settings, color: 'text-canvas-muted-deep' },
};

const getFileIcon = (name: string) => {
  const ext = name.split('.').pop()?.toLowerCase() || '';
  return fileIconMap[ext] || { icon: FileText, color: 'text-canvas-muted-deep' };
};
...
const EditorTabs: React.FC<EditorTabsProps> = ({ onTabSelect, onTabClose }) => {
  ...
  return (
    <div className="flex items-center bg-canvas-card border-b border-canvas-border overflow-x-auto custom-scrollbar h-[38px] shrink-0">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <motion.button
              ...
              className={`
                group relative flex items-center gap-1.5 px-3 h-full text-xs font-medium whitespace-nowrap border-r border-canvas-border transition-colors shrink-0
                ${isActive
                  ? 'bg-canvas-card text-gray-200'
                  : 'bg-transparent text-canvas-muted-deep hover:text-canvas-text hover:bg-white/[0.02]'
                }
              `}
            >
              ...
              {isActive && (
                <motion.div
                  ...
                  className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-primary-600 to-primary-500"
              ...
              {/* Pin indicator */}
              {tab.isPinned && (
                <button onClick={(e) => handlePin(e, tab)} className="p-0.5 text-primary-400 hover:text-primary-300">
                  <Pin className="w-2.5 h-2.5" />
                </button>
              )}

              {/* File icon */}
              <FileIcon className={`w-3.5 h-3.5 ${isActive ? color : 'text-gray-600'} shrink-0 transition-colors`} />
              ...
              {/* Dirty indicator */}
              {tab.isDirty && (
                <div className="w-2 h-2 rounded-full bg-primary-400/80 shrink-0" />
              )}
