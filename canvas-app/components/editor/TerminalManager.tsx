...
const TerminalManager: React.FC<TerminalManagerProps> = ({ className = '' }) => {
  ...
  return (
    <div className={`flex flex-col h-full bg-canvas-card ${className}`}>
      {/* Terminal tab bar */}
      <div className="flex items-center h-[32px] bg-canvas-card border-b border-canvas-border px-1 shrink-0">
            ...
            <button
              ...
              className={`group flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-medium whitespace-nowrap transition-all ${
                activeSessionId === session.id
                  ...
                  : 'text-canvas-muted-deep hover:text-canvas-text hover:bg-white/[0.03]'
              }`}
            >
        ...
        <button
          ...
          className="p-1.5 rounded-md text-canvas-muted-deep hover:text-primary-400 hover:bg-primary-500/10 transition-all shrink-0 ml-1"
          title="New Terminal"
        >
        ...
        <AnimatePresence mode="wait">
          {activeSessionId && sessions.find((s) => s.id === activeSessionId) ? (
            ...
            >
              ...
              <button
                onClick={handleCreate}
                className="px-4 py-2 text-xs font-medium bg-primary-500/10 border border-primary-500/20 text-primary-400 rounded-lg hover:bg-primary-500/20 transition-all"
              >
