...
const colorMap: Record<ToastType, { bg: string; border: string; icon: string; glow: string }> = {
  ...
  error: { bg: 'from-primary-500/10 to-primary-600/5', border: 'border-primary-500/20', icon: 'text-primary-400', glow: 'shadow-primary-500/10' },
  warning: { bg: 'from-amber-500/10 to-amber-600/5', border: 'border-amber-500/20', icon: 'text-amber-400', glow: 'shadow-amber-500/10' },
  info: { bg: 'from-primary-600/10 to-primary-500/5', border: 'border-primary-500/20', icon: 'text-primary-400', glow: 'shadow-primary-500/10' },
};
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3 relative z-10">
        <div className={`w-9 h-9 rounded-xl bg-white/5 border border-canvas-border flex items-center justify-center shrink-0 ${colors.icon}`}>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-white/5">
        <motion.div
          ...
          className={`h-full bg-gradient-to-r ${
            item.type === 'success' ? 'from-emerald-400 to-cyan-400' :
            item.type === 'error' ? 'from-primary-500 to-primary-400' :
