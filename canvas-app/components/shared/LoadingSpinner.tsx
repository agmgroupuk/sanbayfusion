...
const SpinVariant: React.FC<{ size: SpinnerSize }> = ({ size }) => (
  <div className={`${sizeMap[size].container} relative`}>
    <div className={`${sizeMap[size].container} border-2 border-canvas-border rounded-full`} />
    <div className={`${sizeMap[size].container} absolute inset-0 border-2 border-transparent border-t-primary-400 border-r-cyan-400 rounded-full animate-spin`} />
    <div className="absolute inset-[3px] rounded-full bg-gradient-to-br from-primary-600/5 to-primary-500/5" />
  </div>
);
...
const PulseVariant: React.FC<{ size: SpinnerSize }> = ({ size }) => (
  <div className={`${sizeMap[size].container} relative flex items-center justify-center`}>
    <motion.div
      ...
      className={`absolute ${sizeMap[size].container} rounded-full bg-primary-500/20`}
    />
    <motion.div
      animate={{ scale: [1, 1.3, 1], opacity: [0.7, 0.1, 0.7] }}
      transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut', delay: 0.2 }}
      className={`absolute ${sizeMap[size].container} rounded-full bg-cyan-500/20`}
    />
    <div className="w-2 h-2 rounded-full bg-gradient-to-r from-primary-500 to-primary-400" />
  </div>
);
  ...
  <div className="flex items-center gap-1">
    {[0, 1, 2].map((i) => (
      <motion.div
        ...
        className={`${sizeMap[size].dot} rounded-full bg-gradient-to-r from-primary-500 to-primary-400`}
      />
    ))}
  </div>
);
...
const OrbitVariant: React.FC<{ size: SpinnerSize }> = ({ size }) => (
  <div className={`${sizeMap[size].container} relative`}>
    ...
    >
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full bg-primary-400 shadow-lg shadow-primary-500/50" />
    </motion.div>
    <motion.div
      animate={{ rotate: -360 }}
      ...
      className={`${sizeMap[size].container} absolute inset-0`}
    >
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-cyan-400 shadow-lg shadow-cyan-500/50" />
    </motion.div>
  ...
  <div className="flex items-end gap-0.5 h-6">
    {[0, 1, 2, 3, 4].map((i) => (
      <motion.div
        ...
        className="w-1 rounded-full bg-gradient-to-t from-primary-600 to-primary-500"
        style={{ minHeight: 3 }}
      />
    ))}
  </div>
...
const LoadingSpinner: React.FC<LoadingSpinnerProps> = ({
  ...
  return (
    <div className={`inline-flex flex-col items-center gap-3 ${className}`}>
      ...
      {label && (
        <p className={`${sizeMap[size].text} text-canvas-muted-deep font-medium animate-pulse`}>{label}</p>
      )}
    </div>
  );
};
...
export const Skeleton: React.FC<{
  ...
  <div
    className={`bg-white/[0.04] ${rounded} relative overflow-hidden ${className}`}
    style={{ width, height }}
  >
    <motion.div
      animate={{ x: ['-100%', '100%'] }}
