 ...
 * NO localStorage — 100% server-side persistence.
 */
...
// ==================== DEPLOYMENTS LIST / DELETE / DOMAINS ====================
...
/** List all live deployments for the current user */
export async function listDeployments(): Promise<LiveDeployment[]> {
  try {
    const res = await fetch('/api/canvas/deployments', {
      credentials: 'include',
      headers: { 'X-Canvas-Source': SOURCE },
    });
    if (!res.ok) return [];
    const data = await res.json();
    return data.success ? data.deployments : [];
  } catch {
    return [];
  }
}

/** Delete a deployment by its ID/slug */
export async function deleteDeployment(deploymentId: string): Promise<{ success: boolean; error?: string }> {
  try {
    const res = await fetch(`${DEPLOY_API}/${encodeURIComponent(deploymentId)}`, {
      method: 'DELETE',
      credentials: 'include',
      headers: { 'X-Canvas-Source': SOURCE },
    });
    ...
    return { success: false, error: e.message || 'Failed to delete deployment' };
  }
}
...
export async function addCustomDomain(
  deploymentId: string,
  domain: string
): Promise<{ success: boolean; message?: string; dnsRecords?: { type: string; name: string; value: string }[]; error?: string }> {
  try {
    const res = await fetch(`${DEPLOY_API}/domain`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Canvas-Source': SOURCE },
      credentials: 'include',
