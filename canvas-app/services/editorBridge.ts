...
try { window.localStorage.removeItem('editor-bridge-storage'); } catch (_) { }
  ...
  subscribeWithSelector(
    (set, get) => ({
      ...
      loadFromCode: (code, language = 'html') => {
        ...
        // Detect if code is actually HTML content regardless of language tag
        const trimmed = code.trimStart();
        const isActuallyHtml = trimmed.startsWith('<!DOCTYPE') || trimmed.startsWith('<html') ||
          (trimmed.startsWith('<') && (trimmed.includes('</html>') || trimmed.includes('</head>') || trimmed.includes('</body>')));

        if (language === 'html' || isActuallyHtml) {
      ...
      saveToBackend: async () => {
        ...
        try {
          ...
          if (state.currentProjectId) {
            // Update existing project — fallback to create if 404
            try {
              ...
              if (updateErr?.message?.includes('404')) {
                console.log('[EditorBridge] Update 404 — creating new project instead');
