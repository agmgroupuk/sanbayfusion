...
type TabId = 'generate' | 'upload' | 'edit' | 'presets' | 'timeline' | 'export';
...
export function useVideoEditor(_userId?: string) {
    ...
    const planEdit = useCallback(async (prompt: string) => {
        setIsPlanning(true);
        try {
            const resp = await fetch(`${API_BASE}/api/video/plan`, {
                method: 'POST',
    ...
    const executePlan = useCallback(async () => {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            try {
                const resp = await fetch(`${API_BASE}/api/video/execute-step`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
                    body: JSON.stringify({
