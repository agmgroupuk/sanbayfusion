      ...
      {isOpen && (
        <div className="fixed inset-0 z-[9998] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className={`relative w-full ${sizeMap[size]} bg-canvas-card/95 backdrop-blur-2xl border border-canvas-border rounded-2xl shadow-2xl shadow-black/50 overflow-hidden ${className}`}
          >
            ...
            {(title || showCloseButton) && (
              <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-canvas-border">
                <div className="flex items-center gap-3">
                  {icon && (
                    <div className="w-10 h-10 bg-gradient-to-br from-violet-500/20 to-cyan-500/20 border border-violet-500/20 rounded-xl flex items-center justify-center text-violet-400">
                      {icon}
                    </div>
                  )}
                  <div>
            ...
            {/* Footer */}
            {footer && (
              <div className="px-6 py-4 border-t border-canvas-border flex items-center justify-end gap-3 bg-white/[0.02]">
                {footer}
              </div>
            )}
          </motion.div>
...
export const ModalButton: React.FC<{
  ...
  const variants = {
    primary: 'bg-gradient-to-r from-violet-500 to-cyan-500 text-white hover:from-violet-600 hover:to-cyan-600 shadow-lg shadow-violet-500/20',
    secondary: 'bg-white/5 border border-canvas-border text-white/70 hover:text-white hover:bg-white/10',
    danger: 'bg-primary-500/10 border border-primary-500/20 text-primary-400 hover:bg-primary-500/20',
