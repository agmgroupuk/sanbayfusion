...
const severityConfig = {
  error: { icon: XCircle, color: 'text-primary-400', bg: 'bg-primary-500/10', badge: 'bg-primary-500/20 text-primary-400' },
...
const ProblemPanel: React.FC<ProblemPanelProps> = ({ onNavigate, className = '' }) => {
  ...
  return (
    <div className={`h-full bg-canvas-card flex flex-col ${className}`}>
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-canvas-border shrink-0">
        <div className="flex items-center gap-3">
          <span className="text-xs font-semibold text-canvas-text">Problems</span>
          <div className="flex items-center gap-1.5">
            {errorCount > 0 && (
              <span className="flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-primary-500/15 text-primary-400">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {problems.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-gray-600">
            ...
            <p className="text-[10px] text-gray-600 mt-1">Your code looks clean ✨</p>
          </div>
        ) : (
          <AnimatePresence>
            {Array.from(grouped.entries()).map(([file, items]) => (
              <motion.div
                ...
                className="border-b border-canvas-border"
              >
                {/* File header */}
                <div className="flex items-center gap-2 px-3 py-1.5 bg-white/[0.02]">
                  <FileCode className="w-3.5 h-3.5 text-canvas-muted-deep" />
                  <span className="text-[11px] font-medium text-canvas-text truncate">{file}</span>
                ...
                {items.map((problem) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full flex items-start gap-2 px-3 py-1.5 pl-7 hover:bg-white/[0.03] transition-colors text-left group"
                    >
                      <Icon className={`w-3.5 h-3.5 ${config.color} shrink-0 mt-0.5`} />
                      <div className="flex-1 min-w-0">
                        <p className="text-[11px] text-canvas-text leading-relaxed">{problem.message}</p>
                        <p className="text-[10px] text-gray-600 mt-0.5">
                          {problem.source && <span className="text-canvas-muted-deep">[{problem.source}] </span>}
                          Ln {problem.line}, Col {problem.column}
                        </p>
                      </div>
                      <ChevronRight className="w-3 h-3 text-gray-700 opacity-0 group-hover:opacity-100 transition-opacity shrink-0 mt-0.5" />
                    </button>
                  );
                })}
              </motion.div>
