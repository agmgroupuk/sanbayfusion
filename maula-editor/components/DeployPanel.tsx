...
import { vercelDeploy } from '../services/deploy';
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      if (selectedProvider === 'vercel') {
        ...
        throw new Error(`${providers.find(p => p.id === selectedProvider)?.name} deployment requires an API token. Currently Vercel is the supported provider — select Vercel to deploy now.`);
      }
    } catch (err: any) {
