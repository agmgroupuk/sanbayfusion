...
export const Preview = forwardRef<any, PreviewProps>(({ 
  ...
  const handleZoomChange = async (value: number) => {
    ...
    if (track) {
      try {
        ...
        log.warn('Failed to apply zoom:', e);
      }
    }
  };
  ...
  const handleFocusChange = async (value: number) => {
    ...
    if (track) {
      try {
        ...
        log.warn('Failed to apply focus:', e);
      }
    }
  };
  ...
  const resetFocus = async () => {
    ...
    if (track) {
      try {
        ...
        log.warn('Failed to reset focus:', e);
      }
    }
  };
  ...
  return (
    <div className={`bg-vscode-panel flex flex-col h-screen shrink-0 relative overflow-hidden transition-all duration-500 ${isMaximized ? 'flex-1' : 'w-[480px] border-l border-vscode-border'}`}>
      <div className="p-4 border-b border-vscode-border bg-vscode-sidebar flex items-center justify-between sticky top-0 z-50 transition-colors">
        <div className="flex items-center gap-3">
          <span className="text-xl">{app.icon}</span>
          <h2 className="font-bold text-vscode-text text-sm tracking-tight">
            {isMobile ? 'Mobile Preview' : 'Active Preview'}
          </h2>
        </div>
        <button onClick={onToggleMaximize} className="p-2 hover:bg-vscode-hover rounded-lg text-vscode-textMuted">
          {isMaximized ? <Icons.LayoutSplit /> : <Icons.Maximize />}
        </button>
      </div>

      <div className={`flex-1 overflow-y-auto transition-colors ${isMobile ? 'flex items-center justify-center p-8 bg-vscode-bg' : 'p-4'}`}>
        <div className={`flex flex-col h-full transition-all duration-500 ${
          isMobile 
            ? 'w-[320px] h-[640px] bg-vscode-panel rounded-[3rem] border-[8px] border-vscode-border shadow-2xl relative overflow-hidden shrink-0' 
            : 'w-full h-full'
        }`}>
          <div className="flex-1 overflow-y-auto p-4 space-y-4 flex flex-col" ref={scrollRef}>
            {messages.length === 0 && !isCameraActive && (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8 opacity-40">
                ...
                <p className="text-xs font-bold uppercase tracking-widest leading-tight text-vscode-text">System Online<br/>Ask me to build or modify anything.</p>
              </div>
            )}
            ...
            <div className="space-y-4 flex-1">
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2`}>
                  <div className={`max-w-[90%] px-4 py-3 rounded-2xl text-xs ${msg.role === 'user' ? 'bg-vscode-accent text-white shadow-md' : 'bg-vscode-sidebar border border-vscode-border text-vscode-text shadow-sm'}`}>
              ...
              {isLoading && <div className="flex justify-start"><div className="bg-vscode-sidebar border border-vscode-border p-3 rounded-2xl flex gap-1"><div className="w-1.5 h-1.5 bg-vscode-accent rounded-full animate-bounce"></div><div className="w-1.5 h-1.5 bg-vscode-accent rounded-full animate-bounce [animation-delay:0.2s]"></div><div className="w-1.5 h-1.5 bg-vscode-accent rounded-full animate-bounce [animation-delay:0.4s]"></div></div></div>}
            </div>
          </div>

          <div className={`p-4 bg-vscode-sidebar border-t border-vscode-border ${isMobile ? 'pb-8' : ''} transition-colors`}>
            <div className="flex items-center gap-2">
              <input 
                ...
                className="flex-1 bg-vscode-bg border border-vscode-border rounded-xl px-4 py-3 text-xs focus:ring-1 focus:ring-vscode-accent outline-none text-vscode-text" 
              />
              <button onClick={handleSend} disabled={isLoading} className="bg-vscode-accent text-white w-10 h-10 rounded-xl shadow-lg flex items-center justify-center shrink-0 active:scale-95 transition-transform">
                <Icons.Play />
              </button>
            </div>
          </div>
