...
const log = createLogger('InlineCodeSuggestion');
...
export const InlineCodeSuggestion: React.FC<InlineSuggestionProps> = ({
  ...
  const fetchSuggestion = useCallback(async () => {
    ...
    try {
      ...
      log.error('[InlineSuggestion] Error:', error);
      setSuggestion(null);
      setIsVisible(false);
    } finally {
      setIsLoading(false);
...
export function useCopilot() {
  const [isEnabled, setIsEnabled] = useState(() => {
    return memoryStorage.getItem('copilot_enabled') !== 'false';
  });
  const [provider, setProvider] = useState(() => {
    return memoryStorage.getItem('copilot_provider') || 'openai';
  });
  const [isLoading, setIsLoading] = useState(false);
  ...
  const toggleEnabled = useCallback(() => {
    setIsEnabled(prev => {
      const newValue = !prev;
      memoryStorage.setItem('copilot_enabled', String(newValue));
      return newValue;
    });
  }, []);
  ...
  const updateProvider = useCallback((newProvider: string) => {
    ...
    memoryStorage.setItem('copilot_provider', newProvider);
  }, []);
