...
import { getSecret, setSecret } from '../services/secretsService';
...
const PROVIDERS: Array<{ id: AIProvider; name: string; icon: string }> = [
  { id: 'openai', name: 'Smart Engine', icon: '🤖' },
  { id: 'anthropic', name: 'Code Expert', icon: '🧠' },
  { id: 'gemini', name: 'Vision Engine', icon: '✨' },
  { id: 'groq', name: 'Speed Engine', icon: '⚡' },
  { id: 'xai', name: 'Reasoning Engine', icon: '🚀' },
  { id: 'mistral', name: 'Logic Engine', icon: '🌬️' },
  { id: 'ollama', name: 'Local Engine', icon: '🦙' },
];
...
export const CopilotChat: React.FC<CopilotChatProps> = ({ onClose }) => {
  ...
  // Load saved API keys from encrypted backend
  useEffect(() => {
    getSecret('copilot_api_key', provider).then(savedKey => {
      if (savedKey) {
        setApiKey(savedKey);
      }
    });
  ...
  const handleFileOperation = useCallback((operation: FileOperation) => {
    ...
    log.info('[Copilot] File operation:', operation.type, operation.path);
    ...
    try {
      ...
      log.error('[Copilot] Operation error:', error);
    }
  ...
  const handleCommand = useCallback((command: TerminalCommand) => {
    log.info('[Copilot] Terminal command:', command.command);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      log.error('[Copilot] Chat error:', error);
      setIsStreaming(false);
    }
  };
  ...
  // Save API key — store in encrypted backend via secretsService
  const handleSaveApiKey = async () => {
    await setSecret('copilot_api_key', provider, apiKey, `${provider} Copilot API key`);
        ...
        {messages.map((message) => (
          ...
          >
            ...
            <div className={`flex-1 max-w-[85%] ${message.role === 'user' ? 'text-right' : ''}`}>
              <div className={`inline-block text-left px-3 py-2 rounded-lg text-sm ${message.role === 'user'
                ? 'bg-blue-600 text-white'
              ...
              {message.fileOperations && message.fileOperations.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1">
                  {message.fileOperations.map((op, i) => (
                    <span
                      key={i}
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs ${op.type === 'create' ? 'bg-green-900/30 text-green-400' :
