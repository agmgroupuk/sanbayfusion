...
const log = createLogger('FileProjectManager');
...
export const FileProjectManager: React.FC<FileProjectManagerProps> = ({
  ...
  const syncFilesFromServer = useCallback(async () => {
    ...
    setIsSyncing(true);
    try {
      log.info('[FileManager] Syncing files from server for project:', currentProject.id);
      ...
      if (projectData?.files) {
        ...
        setFiles(fileTree);
        log.info('[FileManager] Synced', convertedFiles.length, 'files from server');
      }
    } catch (error) {
      log.error('[FileManager] Failed to sync files:', error);
    } finally {
      setIsSyncing(false);
    }
