...
const log = createLogger('AgenticAIChat.tsx');
...
export const AgenticAIChat: React.FC<AgenticAIChatProps> = ({ 
  ...
  // NOTE: Currently uses local Zustand store mutations. For full backend-driven
  // execution, this should migrate to the /api/canvas/stream SSE agent tool loop
  // (same system used by canvas-studio, gen-craft-pro, neural-chat/canvas-app).
  // The AI currently sends file ops as XML tags in text (<file_create>, <file_edit>,
  // <file_delete>) which are parsed client-side. The target architecture is:
  // Backend defines tools → Model uses tool_use blocks → Backend executes → SSE events.
  ...
  const saveProjectToHistory = useCallback(async (prompt: string) => {
    ...
    log.info(`[AI] Project saved to history: ${projectName}`);
  }, [createProject]);
