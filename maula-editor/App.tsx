...
import { usePreferencesSync } from './services/preferencesSync';
...
const App: React.FC = () => {
  ...
  // DB-backed preferences sync (replaces all localStorage). Hydrates store from
  // /api/editor/{preferences,extensions,workspaces} once authenticated, then
  // debounced-saves any changes back. No localStorage anywhere.
  usePreferencesSync(isAuthenticated && !!user);
  ...
  const editorBridge = useEditorBridge({
    onMessage: (message) => {
      log.info('[EditorBridge] Message:', message);
    ...
    onApprovalRequest: async (action, details) => {
      log.info('[EditorBridge] Approval requested:', action, details);
      // Auto-approve for now, can add UI prompt later
      return true;
    },
    onQuestion: async (question) => {
      log.info('[EditorBridge] Question:', question);
    ...
    onError: (error) => {
      log.error('[EditorBridge] Error:', error);
      setEditorBridgeMessages(prev => [...prev.slice(-99), {
        type: 'error',
        content: error,
        timestamp: Date.now()
  ...
  useEffect(() => {
    ...
    log.info('[EditorBridge] Initialized and exposed globally');
    return () => {
