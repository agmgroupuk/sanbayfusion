...
const log = createLogger('vcsExtension.ts');
/**
...
class VCSExtensionRegistry {
  ...
  register(type: VCSType, factory: () => VCSProvider): void {
    ...
    log.info(`[VCS] Registered provider: ${type}`);
  }
  ...
  unregister(type: VCSType): void {
    ...
    log.info(`[VCS] Unregistered provider: ${type}`);
  }
  ...
  getProvider(type: VCSType): VCSProvider | null {
    ...
    if (!factory) {
      log.warn(`[VCS] No provider registered for type: ${type}`);
      return null;
    }
  ...
  private notifyListeners(type: VCSType, provider: VCSProvider | null): void {
    for (const listener of this.listeners) {
      try {
        ...
        log.error('[VCS] Listener error:', err);
      }
    }
  }
}
