...
const log = createLogger('gitPro.ts');
...
class GitService {
  ...
  async init(options?: { defaultBranch?: string }): Promise<void> {
    ...
    log.info(`Git repository initialized with default branch: ${defaultBranch}`);
  }
  ...
  async clone(options: GitCloneOptions): Promise<void> {
    ...
    log.info(`Cloned ${options.url} to ${dir}`);
  }
  ...
  async commit(message: string, options?: {
    ...
    log.info(`Committed: ${sha.slice(0, 7)} - ${message}`);
    return sha;
  }
  ...
  async listBranches(options?: { remote?: string }): Promise<GitBranch[]> {
    try {
      ...
      log.error('List branches error:', error);
      return [];
    }
  }
  ...
  private loadFromStorage(): void {
    try {
      ...
      log.error('Failed to load git data from storage:', error);
    }
  }
  ...
  private saveToStorage(): void {
    try {
      ...
      log.error('Failed to save git data to storage:', error);
    }
  }
