...
const log = createLogger('extensions.ts');
  ...
  emit: (event: string, ...args: any[]): void => {
    eventHandlers.get(event)?.forEach(handler => {
      try {
        ...
        log.error(`Extension event handler error (${event}):`, error);
      }
    });
  },
};
...
export const extensionManager = {
  ...
  register: (extension: Extension): void => {
    if (registeredExtensions.has(extension.id)) {
      log.warn(`Extension ${extension.id} is already registered`);
      return;
    }
    ...
    log.info(`✨ Extension registered: ${extension.name}`);
  ...
  unregister: (extensionId: string): void => {
    ...
    log.info(`Extension unregistered: ${extension.name}`);
...
export const initializeExtensions = (): void => {
  ...
  log.info(`✨ Initialized ${builtInExtensions.length} built-in extensions`);
};
  ...
  const commands: ExtensionCommand[] = config.commands?.map(cmd => ({
    ...
    handler: () => {
      try {
        ...
        log.error(`Extension command error (${cmd.id}):`, error);
      }
    },
  })) || [];
