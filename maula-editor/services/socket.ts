...
class SocketService {
  ...
  connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.socket.on('connect_error', (error) => {
        log.warn('[Socket] Connection error (continuing offline):', error.message);
        this.reconnectAttempts++;
        if (this.reconnectAttempts >= this.maxReconnectAttempts) {
          log.warn('[Socket] Max reconnection attempts reached, continuing in offline mode');
          this.connected = false;
  ...
  createTerminal(options?: {
    ...
    return new Promise((resolve, reject) => {
      if (!this.socket?.connected) {
        ...
        log.warn('[Socket] Creating offline terminal:', fakeTerminalId);
        return resolve(fakeTerminalId);
      }

      log.info('[Socket] Creating terminal with options:', options);
      this.socket.emit('terminal:create', options);

      this.socket.once('terminal:created', (data: { terminalId: string }) => {
        log.info('[Socket] Terminal created:', data.terminalId);
  ...
  sendTerminalInput(terminalId: string, input: string): void {
    if (!this.socket?.connected) {
      log.warn('[Socket] Cannot send input - offline mode');
      return;
    }
  ...
  resizeTerminal(terminalId: string, cols: number, rows: number): void {
    if (!this.socket?.connected) {
      log.warn('[Socket] Cannot resize terminal - offline mode');
      return;
    }
  ...
  killTerminal(terminalId: string): void {
    if (!this.socket?.connected) {
      log.warn('[Socket] Cannot kill terminal - offline mode');
      return;
    }
  ...
  requestCodeCompletion(
    ...
    if (!this.socket?.connected) {
      log.warn('[Socket] Not connected for code completion');
      return;
    }
  ...
  getAgentList(callback: (agents: Array<{ id: string; name: string; icon: string; description: string }>) => void): void {
    if (!this.socket?.connected) {
      log.warn('[Socket] Not connected for agent list');
      return;
    }
  ...
  clearAgentHistory(callback?: () => void): void {
    if (!this.socket?.connected) {
      log.warn('[Socket] Not connected for clearing agent history');
      return;
    }
