...
const log = createLogger('speech.ts');
...
export const voiceInput = {
  ...
  start: (callbacks: VoiceInputCallbacks, options?: {
    ...
    recognitionInstance.onresult = (event: any) => {
      ...
      if (isFinal) {
        log.info(`🎤 Final: "${transcript}" (${Math.round(confidence * 100)}% confidence)`);
      }
    };
