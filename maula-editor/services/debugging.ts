...
const log = createLogger('debugging.ts');
  ...
  connectToServer(): void {
    ...
    try {
      ...
      import('socket.io-client').then(({ io }) => {
        ...
        log.info('🐛 Debug server not available, using simulation mode');
        this.useServerMode = false;
      });

      // Listen for debug events from server
      this.socket.on('debug:created', (data) => {
        log.info('Debug session created:', data);
      ...
      this.socket.on('debug:error', (data) => {
        log.error('Debug error:', data.error);
      });

      this.socket.on('debug:evaluated', (data) => {
        log.info('Evaluation result:', data);
      });
      ...
      this.socket.on('debug:breakpointSet', (data) => {
        ...
        log.info('🐛 Socket.io-client not available, using simulation mode');
        this.useServerMode = false;
      });
    } catch (error) {
      log.error('Failed to connect to debug server:', error);
      this.useServerMode = false;
    }
  }
