...
const log = createLogger('aiExtension.ts');
/**
 * AI Extension Layer
 * 
...
class AIExtensionController {
  ...
  connect(capabilities: EditorCapabilities) {
    ...
    log.info('[AI Extension] Connected to editor capabilities');
  }
  ...
  setPermissionLevel(level: PermissionLevel) {
    ...
    log.info(`[AI Extension] Permission level set to: ${level}`);
  }
    ...
    try {
      ...
      if (plan) {
        ...
        if (!validationResult.valid) {
          log.error('[AI Extension] Plan validation failed:', validationResult.errors);
          return null;
        }
      ...
      log.error('[AI Extension] AI request failed:', error.message);
      return null;
    }
  }
...
## Your Response (JSON only)`;
  ...
  private parseAIResponse(content: string): AIActionPlan | null {
    try {
      ...
      if (!jsonMatch) {
        log.error('[AI Extension] No JSON found in response');
        return null;
      }
      ...
      log.error('[AI Extension] Failed to parse AI response:', error);
      return null;
    }
  }
