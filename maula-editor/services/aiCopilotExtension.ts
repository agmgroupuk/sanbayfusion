...
const log = createLogger('aiCopilotExtension.ts');
/**
...
class AICopilotExtension {
  ...
  connectToEditor(editor: any, monaco: any) {
    ...
    log.info('[AI Copilot] Connected to editor');
  }
  ...
  private async requestCompletion(context: CompletionContext) {
    ...
    try {
      ...
      if (this.state.lastRequestId !== requestId || this.abortController.signal.aborted) {
        log.info('[AI Copilot] Request outdated, discarding');
        return;
      }
      ...
      if (suggestion) {
        ...
        if (this.isCursorAtExpectedPosition(context)) {
          ...
          log.info('[AI Copilot] Cursor moved, discarding suggestion');
        }
      }
      
    } catch (error: any) {
      // Silent failure - AI is optional
      log.error('[AI Copilot] Request error:', error.message);
