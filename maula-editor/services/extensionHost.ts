...
const log = createLogger('extensionHost.ts');
...
class ExtensionHostService {
  ...
  async activateExtension(manifest: ExtensionManifest): Promise<boolean> {
    ...
    if (this.runtimes.has(id) && this.runtimes.get(id)!.status === 'active') {
      log.info(`[ExtensionHost] Extension ${id} already active`);
      return true;
    }

    log.info(`[ExtensionHost] Activating extension: ${id}`);
    ...
    try {
      ...
      worker.onmessage = async (e) => {
        ...
        if (type === 'api-call') {
          ...
          log.info(`[${id}]`, ...data.args);
        } else if (type === 'error') {
          log.error(`[${id}] Error:`, data.message);
          extensionEvents.emit('extension:error', { extensionId: id, error: data.message });
        } else if (type === 'activated') {
          runtime.status = 'active';
          log.info(`[ExtensionHost] Extension ${id} activated successfully`);
      ...
      worker.onerror = (error) => {
        log.error(`[ExtensionHost] Worker error for ${id}:`, error);
      ...
      log.error(`[ExtensionHost] Failed to activate ${id}:`, error);
      runtime.status = 'error';
      runtime.error = error.message;
      return false;
    }
  ...
  if (type === 'activate') {
    ...
    if (typeof module !== 'undefined' && module.exports && module.exports.deactivate) {
      try {
        ...
        log.error('Deactivate error:', err);
      }
    }
    self.close();
  }
...
`;
  ...
  async deactivateExtension(extensionId: string): Promise<void> {
    ...
    log.info(`[ExtensionHost] Deactivating extension: ${extensionId}`);
  ...
  async reloadExtension(manifest: ExtensionManifest): Promise<boolean> {
    log.info(`[ExtensionHost] Hot reloading extension: ${manifest.id}`);
...
export function connectEditorToExtensions(editor: any, monaco: any) {
  ...
  log.info('[ExtensionHost] Editor connected');
}
...
export function connectStoreToExtensions(store: any) {
  ...
  log.info('[ExtensionHost] Store connected');
}
