...
export class FilesService {
  ...
  private async syncToFilesystem(projectId: string, filePath: string, content: string, type: string): Promise<void> {
    ...
    try {
      ...
      log.info(`[Files] Synced to filesystem: ${fullPath}`);
    } catch (error) {
      log.error(`[Files] Failed to sync to filesystem: ${fullPath}`, error);
    }
  }
  ...
  private async deleteFromFilesystem(projectId: string, filePath: string): Promise<void> {
    ...
    try {
      ...
      log.info(`[Files] Deleted from filesystem: ${fullPath}`);
    } catch (error) {
      log.error(`[Files] Failed to delete from filesystem: ${fullPath}`, error);
    }
  }
