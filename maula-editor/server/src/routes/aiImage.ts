...
const log = createLogger('aiImage');
...
router.post('/generate', async (req: Request, res: Response) => {
  try {
    ...
    log.info(`[AI Image] Generating with ${model}: "${prompt.substring(0, 50)}..."`);
      ...
      (response.data || []).map(async (img, index) => {
        ...
        if (isS3Configured() && img.url) {
          try {
            ...
            log.error('[AI Image] Failed to save to S3:', s3Error);
    ...
    log.info(`[AI Image] Generated ${images.length} image(s)`);
...
router.post('/edit', upload.fields([
  ...
  try {
    ...
    log.info(`[AI Image] Editing image: "${prompt.substring(0, 50)}..."`);
    ...
    log.info(`[AI Image] Edited ${images.length} image(s)`);
...
router.post('/variation', upload.single('image'), async (req: Request, res: Response) => {
  try {
    ...
    log.info('[AI Image] Creating variations');
    ...
    log.info(`[AI Image] Created ${images.length} variation(s)`);
