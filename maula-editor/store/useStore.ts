...
const log = createLogger('useStore');
...
function debouncedSaveFile(fileId: string, content: string) {
  ...
  _fileSaveTimers.set(fileId, setTimeout(() => {
    ...
    filesApiService.updateFile(fileId, content).catch(e => log.warn('[Store] Auto-save failed:', e));
  }, 1500));
}
  ...
  (set, get) => ({
      ...
      createProject: async (name, template, files) => {
        ...
        try {
          ...
          log.info('[Store] Project created on backend:', backendProject.id);
          ...
          for (const file of files) {
            if (file.type === 'file') {
              try {
                ...
                log.warn('[Store] Failed to sync file:', file.path, e);
              }
            }
          }
        } catch (error) {
          log.warn('[Store] Backend sync failed, using local project:', error);
        }
      ...
      createFile: (path, name, content = '') => {
        ...
        if (projectId) {
          ...
          }).catch(e => log.warn('[Store] Backend createFile failed:', e));
        }
      },
      ...
      deleteNode: (path) => {
        ...
        if (node?.id) {
          filesApiService.deleteFile(node.id).catch(e => log.warn('[Store] Backend deleteFile failed:', e));
        }
      },
