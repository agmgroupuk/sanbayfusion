...
const log = createLogger('FilesRoute');
/**
  ...
  try {
    ...
    if (mainSiteSessionId) {
      try {
        ...
        log.warn('[Files Auth] Main site session check failed:', mainSiteErr.message);
      }
    }
  ...
  try {
    ...
    if (ext === 'pdf' || mimeType === 'application/pdf') {
      try {
        ...
        log.error('[Files] PDF extraction error:', pdfError);
    ...
    if (ext === 'docx' || mimeType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
      try {
        ...
        log.error('[Files] DOCX extraction error:', docxError);
        return res.status(500).json({
          success: false,
    ...
    if (ext === 'csv' || mimeType === 'text/csv') {
      try {
        ...
        log.error('[Files] CSV extraction error:', csvError);
        return res.status(500).json({
          success: false,
    ...
    log.error('[Files] Extract error:', error);
