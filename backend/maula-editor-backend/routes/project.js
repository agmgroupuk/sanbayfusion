...
// POST /api/project/:id/git/stage  — stage files explicitly
router.post('/:id/git/stage', requireAuth, async (req, res) => {
  try {
    const project = await projectRepo.findById(req.params.id);
    if (!project) return res.status(404).json({ error: 'Project not found' });
    if (project.userId !== req.user.id) return res.status(403).json({ error: 'Access denied' });
    ...
    const result = await gitService.stageFiles(project.sandboxId, Array.isArray(files) && files.length ? files : ['.']);
...
// POST /api/project/:id/git/unstage  — unstage files
router.post('/:id/git/unstage', requireAuth, async (req, res) => {
  try {
    const project = await projectRepo.findById(req.params.id);
    if (!project) return res.status(404).json({ error: 'Project not found' });
    if (project.userId !== req.user.id) return res.status(403).json({ error: 'Access denied' });
    ...
    const { files } = req.body || {};
    const result = await gitService.unstageFiles(project.sandboxId, Array.isArray(files) && files.length ? files : ['.']);
    res.json(result || { success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// GET /api/project/:id/git/log  — commit history
router.get('/:id/git/log', requireAuth, async (req, res) => {
  try {
    ...
    const commits = await gitService.getLog(project.sandboxId, limit);
    res.json({ commits });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// POST /api/project/:id/git/checkout  — switch branch
router.post('/:id/git/checkout', requireAuth, async (req, res) => {
  try {
    ...
    const result = await gitService.checkoutBranch(project.sandboxId, branch);
    res.json(result || { success: true, branch });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// POST /api/project/:id/git/remotes  — add remote
router.post('/:id/git/remotes', requireAuth, async (req, res) => {
  try {
    const project = await projectRepo.findById(req.params.id);
    if (!project) return res.status(404).json({ error: 'Project not found' });
    if (project.userId !== req.user.id) return res.status(403).json({ error: 'Access denied' });
    ...
    const result = await gitService.addRemote(project.sandboxId, name, url);
    res.json(result || { success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// GET /api/project/:id/git/diff  — unified diff
router.get('/:id/git/diff', requireAuth, async (req, res) => {
  try {
    const project = await projectRepo.findById(req.params.id);
    if (!project) return res.status(404).json({ error: 'Project not found' });
    if (project.userId !== req.user.id) return res.status(403).json({ error: 'Access denied' });
    ...
    const diff = await gitService.getDiff(project.sandboxId, { filepath });
    res.json({ diff });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
