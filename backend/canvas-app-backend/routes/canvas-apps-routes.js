...
// The canvas database has its own `users` table, but auth comes from the
// main maulaai database.  Before any CRUD that references userId we upsert
// a minimal mirror row so the FK is satisfied.
...
router.get('/', async (req, res) => {
  try {
    ...
    // Allow unauthenticated access (return empty list for demo mode)
    if (!userId) {
      return res.json({ success: true, apps: [], pagination: { page: 1, limit: 50, total: 0, pages: 0 } });
    }
...
router.post('/', async (req, res) => {
  try {
    const userId = await getUserId(req);
    if (!userId) {
      console.warn('[CanvasApps] POST rejected — no userId. cookies:', Object.keys(req.cookies || {}), 'user:', !!req.user);
      return res.status(400).json({ success: false, error: 'Authentication required to save apps' });
    }
...
router.patch('/:id', async (req, res) => {
  try {
    const userId = await getUserId(req);
    if (!userId) {
      return res.status(400).json({ success: false, error: 'Authentication required to update apps' });
    }
    ...
    if (!existing) {
      ...
      // App not yet in DB and no code to create it — return success so the
      // frontend doesn't show an error (the app will be saved on next full save).
      return res.status(200).json({ success: true, app: null, note: 'App not yet persisted' });
    }
...
router.delete('/:id', async (req, res) => {
  try {
    const userId = await getUserId(req);
    if (!userId) {
      return res.status(400).json({ success: false, error: 'Authentication required to delete apps' });
    }
...
router.delete('/', async (req, res) => {
  try {
    const userId = await getUserId(req);
    if (!userId) {
      return res.status(400).json({ success: false, error: 'Authentication required to clear apps' });
    }
