...
export const CANVAS_IDE_TOOLS = [
  ...
  {
    name: 'llm_chat',
    description: 'Multi-provider LLM chat (OpenAI, Anthropic, Groq, Google). Send prompts or message arrays.',
    parameters: {
      type: 'object',
      properties: {
  ...
  // \u2500\u2500 ADDED: Tools that existed in agentToolsService but were missing from AI definitions \u2500\u2500

  {
    name: 'generate_code',
    description: 'Generate new code for a specific purpose (function, class, module, component). Provide language and description.',
    parameters: {
      type: 'object',
      properties: {
        language: { type: 'string', description: 'Programming language (javascript, python, typescript, etc.)' },
        description: { type: 'string', description: 'What the code should do' },
        context: { type: 'string', description: 'Optional context or existing code to build upon' },
        style: { type: 'string', enum: ['function', 'class', 'module', 'component', 'snippet'], description: 'Code structure style' },
      },
  ...
  {
    name: 'lint_code',
    description: 'Lint/check code for errors, style issues, and best practice violations. Returns list of problems.',
    parameters: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'File path to lint' },
        code: { type: 'string', description: 'Code content to lint (if not reading from file)' },
        language: { type: 'string', description: 'Language: javascript, typescript, python, css, html' },
        ruleset: { type: 'string', description: 'Optional ruleset: eslint-recommended, airbnb, standard' },
      },
      required: [],
    },
  },
  {
    name: 'refactor_code',
    description: 'Refactor code to improve quality, performance, or readability without changing behaviour.',
    parameters: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'File path to refactor' },
        goal: { type: 'string', description: 'Refactoring goal: extract_function, rename, simplify, performance, readability' },
        code: { type: 'string', description: 'Code to refactor (alternative to path)' },
      },
      required: ['goal'],
    },
  },
  {
    name: 'test_code',
    description: 'Run tests for a specific file or test suite and return pass/fail results.',
    parameters: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'Test file path or directory' },
        framework: { type: 'string', description: 'Test framework: jest, vitest, pytest, mocha' },
        pattern: { type: 'string', description: 'Optional test name pattern to filter' },
        coverage: { type: 'boolean', description: 'Whether to collect coverage data' },
      },
      required: [],
    },
  },
  {
    name: 'run_in_sandbox',
    description: 'Execute code in an isolated sandbox environment and return the output. Safer than execute_code for untrusted snippets.',
    parameters: {
      type: 'object',
      properties: {
        code: { type: 'string', description: 'Code to execute' },
        language: { type: 'string', description: 'Language: javascript, python, bash' },
        timeout: { type: 'number', description: 'Timeout in milliseconds (default 5000)' },
        input: { type: 'string', description: 'Optional stdin input' },
      },
      required: ['code', 'language'],
    },
  },
  {
    name: 'analyze_video',
    description: 'Analyze a video file: detect scenes, extract metadata, identify content, check quality.',
    parameters: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'Video file path' },
        url: { type: 'string', description: 'Video URL (alternative to path)' },
        tasks: {
          type: 'array',
          items: { type: 'string' },
          description: 'Analysis tasks: metadata, scenes, quality, content, thumbnails',
        },
      },
      required: [],
    },
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'Input video file path' },
        start: { type: 'string', description: 'Start time (e.g. 00:00:10 or 10)' },
        end: { type: 'string', description: 'End time (e.g. 00:01:30 or 90)' },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        output_dir: { type: 'string', description: 'Directory to write frames into' },
        format: { type: 'string', enum: ['jpg', 'png', 'webp'], description: 'Image format' },
      },
      required: ['path'],
    },
  },
  {
    name: 'convert_audio',
    description: 'Convert audio files between formats (mp3, wav, ogg, flac, aac, m4a).',
    parameters: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'Input audio file path' },
        format: { type: 'string', description: 'Target format: mp3, wav, ogg, flac, aac, m4a' },
        bitrate: { type: 'string', description: 'Optional bitrate e.g. 128k, 320k' },
        output: { type: 'string', description: 'Output file path' },
      },
  ...
  {
    name: 'analyze_audio',
    description: 'Analyze an audio file: detect silence, measure volume, get duration and format info.',
    parameters: {
      type: 'object',
      properties: {
        ...
        tasks: {
          type: 'array',
          items: { type: 'string' },
          description: 'Tasks: metadata, silence_detect, loudness, waveform',
        },
      },
      required: ['path'],
    },
  ...
  {
    name: 'edit_image',
    description: 'Edit an image using AI: remove background, inpaint, extend, or apply a text instruction.',
    parameters: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'Image file path' },
        operation: { type: 'string', enum: ['remove_background', 'inpaint', 'extend', 'ai_edit'], description: 'Edit operation' },
        prompt: { type: 'string', description: 'Text instruction for ai_edit or inpaint fill description' },
        mask: { type: 'string', description: 'Mask image path for inpainting' },
        output: { type: 'string', description: 'Output file path' },
      },
  ...
  },
  {
    ...
    description: 'Search the project codebase or uploaded documents using semantic (meaning-based) similarity rather than exact keyword match.',
    parameters: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Natural language search query' },
        top_k: { type: 'number', description: 'Number of results to return (default 5)' },
        scope: { type: 'string', enum: ['files', 'docs', 'both'], description: 'What to search' },
      },
      required: ['query'],
    },
  },
  {
    name: 'store_vectors',
    description: 'Store text content as vector embeddings for later semantic retrieval.',
    parameters: {
      type: 'object',
      properties: {
        texts: { type: 'array', items: { type: 'string' }, description: 'Array of texts to embed and store' },
        namespace: { type: 'string', description: 'Namespace/collection to store into' },
        metadata: { type: 'object', description: 'Optional metadata to attach to each entry' },
      },
      required: ['texts'],
    },
  },
  {
    name: 'embed_content',
    description: 'Generate vector embeddings for text or code content.',
    parameters: {
      type: 'object',
      properties: {
        text: { type: 'string', description: 'Text to embed' },
  ...
  {
    name: 'run_code',
    description: 'Execute a code snippet and return its output. Supports JavaScript and Python.',
    parameters: {
      type: 'object',
      properties: {
        code: { type: 'string', description: 'Code to execute' },
  ...
  {
    name: 'parse_ast',
    description: 'Parse source code into an Abstract Syntax Tree and extract structure (functions, classes, imports, exports).',
    parameters: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'File path to parse' },
        code: { type: 'string', description: 'Code string to parse (alternative to path)' },
        language: { type: 'string', description: 'Language: javascript, typescript, python' },
        extract: { type: 'array', items: { type: 'string' }, description: 'What to extract: functions, classes, imports, exports, variables' },
      },
      required: [],
    },
  },
  {
    name: 'plan_task',
    description: 'Break a complex user goal into a structured list of concrete sub-tasks with dependencies.',
    parameters: {
      type: 'object',
      properties: {
        goal: { type: 'string', description: 'The high-level goal to plan' },
        context: { type: 'string', description: 'Current project context or constraints' },
        max_steps: { type: 'number', description: 'Maximum number of steps (default 8)' },
      },
      required: ['goal'],
    },
  },
  {
    name: 'delegate_task',
    description: 'Delegate a specific sub-task to a specialised capability (code generation, image creation, data analysis).',
    parameters: {
      type: 'object',
      properties: {
        task: { type: 'string', description: 'Task description' },
        capability: { type: 'string', enum: ['code', 'image', 'data', 'search', 'analysis'], description: 'Required capability' },
        context: { type: 'string', description: 'Context to pass to the delegate' },
      },
  ...
  {
    name: 'finalize_task',
    description: 'Mark a task as complete and summarise the work done. Use after finishing a multi-step task.',
    parameters: {
      type: 'object',
      properties: {
        task_id: { type: 'string', description: 'Task identifier' },
        summary: { type: 'string', description: 'Summary of what was accomplished' },
        files_modified: { type: 'array', items: { type: 'string' }, description: 'List of file paths that were changed' },
      },
      required: ['summary'],
    },
  },
  {
    name: 'review_output',
    description: 'Review and critique generated code or content for quality, correctness, and completeness.',
    parameters: {
      type: 'object',
      properties: {
        content: { type: 'string', description: 'Content to review' },
        criteria: { type: 'array', items: { type: 'string' }, description: 'Review criteria: correctness, performance, security, style, completeness' },
        path: { type: 'string', description: 'File path to review (alternative to content)' },
      },
      required: [],
    },
  },
 ...
 */
export function toAnthropicTools(tools = CANVAS_IDE_TOOLS, source = 'standalone') {
...
/**
 * Tools for the chat panel (agent-chat routes).
 * Only what Nova needs to build/edit code — NOT image processing, video, zip,
 * or editor-cursor tools (those have their own dedicated panels).
 */
