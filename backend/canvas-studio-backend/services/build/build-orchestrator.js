 ...
 * Runs builds via direct child_process execution in temp directories (no Docker/sandbox)
 */
...
const BUILD_ROOT = path.join(os.tmpdir(), 'canvas-studio-builds');
...
const BUILD_STAGES = [
  ...
  { id: 'package', name: 'Package Artifacts', timeout: 30000 },
];
...
class BuildOrchestrator {
  constructor() {
    ...
    this._detectedFramework = null;
    // Ensure build root exists
    fs.mkdir(BUILD_ROOT, { recursive: true }).catch(() => { });
  }

  /**
   * Execute a command in a directory, streaming stdout/stderr line-by-line via buildLogger SSE.
   */
  _execInDir(buildId, cwd, cmd, args = [], { timeout = 60000, signal } = {}) {
    return new Promise((resolve) => {
      const child = spawn(cmd, args, {
        cwd,
        shell: true,
        env: { ...process.env, CI: 'true', NODE_ENV: 'production', FORCE_COLOR: '0' },
      ...
      if (active) active.childProcess = child;
