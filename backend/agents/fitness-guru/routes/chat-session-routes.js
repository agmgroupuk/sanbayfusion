...
// since this backend's own database (universal_chat) doesn't store users.
  ...
  try {
    ...
    const session = await prisma.chatSession.create({
      data: {
        ...
        name: title || 'New Chat',
        settings: settings || { temperature: 0.7, maxTokens: 2000, provider: 'openai' },
      },
      include: {
...
// ─────────────────────────────────────────────────────────────────────────────
// PUT /api/sessions/:id/messages/:messageId — Update a message's content
// ─────────────────────────────────────────────────────────────────────────────
router.put('/:id/messages/:messageId', requireAuth, async (req, res) => {
  try {
    ...
    res.json({ success: true, message: { id: updated.id, content: updated.content } });
  } catch (error) {
    console.error('[Sessions] Update message error:', error);
...
router.delete('/:id/messages/:messageId', requireAuth, async (req, res) => {
  try {
    ...
    console.error('[Sessions] Delete message error:', error);
    res.status(500).json({ success: false, error: 'Failed to delete message' });
  }
});

// ─────────────────────────────────────────────────────────────────────────────
// DELETE /api/sessions/:id/messages — Bulk delete all messages in a session
// ─────────────────────────────────────────────────────────────────────────────
router.delete('/:id/messages', requireAuth, async (req, res) => {
  try {
    ...
    const result = await prisma.chatMessage.deleteMany({ where: { sessionId: id } });

    // Reset session stats
    await prisma.chatSession.update({
      where: { sessionId: id },
      data: { stats: { messageCount: 0, totalTokens: 0, durationMs: 0 } },
    });

    res.json({ success: true, deleted: result.count });
  } catch (error) {
    console.error('[Sessions] Bulk delete messages error:', error);
    res.status(500).json({ success: false, error: 'Failed to delete messages' });
  }
});

// ─────────────────────────────────────────────────────────────────────────────
// DELETE /api/sessions/:id/files/:fileName — Delete a single canvas file
// ─────────────────────────────────────────────────────────────────────────────
router.delete('/:id/files/:fileName', requireAuth, async (req, res) => {
  try {
    ...
    const file = await prisma.chatCanvasFile.findFirst({ where: { sessionId: id, fileName: decodeURIComponent(fileName) } });
    if (!file) {
      return res.status(404).json({ success: false, error: 'File not found' });
    }

    await prisma.chatCanvasFile.delete({ where: { id: file.id } });
    res.json({ success: true });
  } catch (error) {
    console.error('[Sessions] Delete file error:', error);
    res.status(500).json({ success: false, error: 'Failed to delete file' });
  }
});

// ─────────────────────────────────────────────────────────────────────────────
// PUT /api/sessions/:id/settings — Update session settings (pins, etc.)
// ─────────────────────────────────────────────────────────────────────────────
router.put('/:id/settings', requireAuth, async (req, res) => {
  try {
    ...
    // Merge with existing settings (Json field)
    const existingSettings = session.settings || {};
    ...
    console.error('[Sessions] Update settings error:', error);
