...
class AgentFileAdapter {
  ...
  _getData() {
    const { _isNew, id, createdAt, updatedAt, ...data } = this;
    // Map agent-tools-service field names to Prisma schema fields
    if (data.size !== undefined && data.fileSize === undefined) {
    ...
    if (!data.fileType && data.mimeType) {
      data.fileType = data.mimeType.split('/').pop() || 'unknown';
    }
    if (data.fileSize === undefined) data.fileSize = 0;
    if (!data.fileName) data.fileName = data.originalName || 'untitled';
    if (!data.fileType) data.fileType = 'unknown';
    return data;
  }
  ...
  static async findOne(query) {
    ...
    // Support Mongoose-style $or queries (used by readFile)
    if (query.$or) {
      where.OR = query.$or.map(cond => {
        const w = {};
        if (cond.path) w.path = cond.path;
        if (cond.filename) w.filename = cond.filename;
        return w;
      });
    }
