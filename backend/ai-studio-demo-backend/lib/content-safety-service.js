...
export async function analyzeText(text, options = {}) {
    if (!isConfigured()) {
        console.warn('[ContentSafety] Not configured — skipping analysis');
        return { safe: true, skipped: true, reason: 'not_configured' };
    }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            // Fail open — don't block users if the safety API is down
            return { safe: true, skipped: true, reason: 'api_error', statusCode: response.status };
        }
        ...
        if (error.name === 'TimeoutError' || error.name === 'AbortError') {
            console.warn('[ContentSafety] Request timed out — allowing message');
            return { safe: true, skipped: true, reason: 'timeout' };
        }
        console.error('[ContentSafety] Error:', error.message);
        // Fail open
        return { safe: true, skipped: true, reason: 'error', error: error.message };
    }
}
...
export async function analyzeImage(base64Content, options = {}) {
    ...
    try {
        ...
        if (error.name === 'TimeoutError' || error.name === 'AbortError') {
            console.warn('[ContentSafety] Image analysis timed out — allowing');
            return { safe: true, skipped: true, reason: 'timeout' };
        }
...
export function contentSafetyMiddleware(options = {}) {
    return async (req, res, next) => {
        ...
        if (!text || !isConfigured()) {
            req.contentSafety = { safe: true, skipped: true };
            return next();
        }
