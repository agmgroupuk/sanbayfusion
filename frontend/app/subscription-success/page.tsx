...
function SuccessContent() {
    ...
    const isCanvasStudio = agentSlug === 'canvas-studio';

    // Subscription data from API
    const [status, setStatus] = useState<'loading' | 'success' | 'error'>('loading');
    const [subData, setSubData] = useState<{ id?: string; plan?: string; price?: number; daysRemaining?: number; status?: string; expiryDate?: string; agentId?: string } | null>(null);
            ...
            <div className="min-h-screen bg-[#030304] flex items-center justify-center px-6">
                <div className="max-w-md text-center">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 justify-center">
                        <Link href="/dashboard" className="px-6 py-3 rounded-xl bg-violet-600/90 text-white font-medium text-sm">Go to Dashboard</Link>
            ...
            <div className="page-content relative z-10 min-h-screen flex items-center justify-center py-20 px-4">
                <div className="max-w-2xl w-full mx-auto text-center">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 justify-center mb-12">
                        <Link href={agentLink} className="action-btn inline-flex items-center justify-center px-8 py-3.5 rounded-xl bg-gradient-to-r from-violet-600/90 to-fuchsia-600/90 text-white font-semibold text-sm shadow-lg shadow-violet-600/15 hover:shadow-violet-600/30 transition-all duration-300 gap-2 hover:scale-[1.02]">
                            <MessageSquare className="w-4 h-4" />{isCanvasStudio ? 'Open Canvas Studio' : 'Start Chatting'}
                        </Link>
                        <Link href="/dashboard" className="action-btn inline-flex items-center justify-center px-8 py-3.5 rounded-xl bg-white/[0.03] border border-white/[0.08] text-gray-400 font-semibold text-sm hover:bg-white/[0.06] hover:text-white hover:border-white/[0.15] transition-all duration-300 gap-2 hover:scale-[1.02]">
