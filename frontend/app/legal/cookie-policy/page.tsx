    ...
    const sections = [
        {
            id: 'introduction', icon: Cookie, title: '1. Introduction & Legal Framework', glow: 'rgba(245,158,11,0.4)', content: [
                { subtitle: 'Scope', text: 'This Cookie Policy explains how One Last AI (\u201cwe,\u201d \u201cour,\u201d or \u201cus\u201d) uses cookies and similar tracking technologies on our websites at sanbayfusion.com and spaces.sanbayfusion.com, including all sub-applications (Canvas App, Canvas Studio, GenCraft Pro, and Maula Editor). By using our website, you consent to our use of cookies in accordance with this policy and our Privacy Policy.' },
        ...
        },
        {
            id: 'third-party', icon: Server, title: '4. Third-Party Services', glow: 'rgba(99,102,241,0.4)', content: [
                { subtitle: 'No Third-Party Tracking Cookies', text: 'We do NOT use Google Analytics, Facebook Pixel, Google Tag Manager, or any third-party tracking/advertising cookies. All analytics are self-hosted in our own PostgreSQL database on the hosting provider infrastructure we control. This is a core design principle \u2014 not an afterthought.' },
                { subtitle: 'the payment provider (Payment Processing)', text: 'PCI DSS Level 1 compliant payment processor. the payment provider may set its own strictly necessary cookies for fraud detection during checkout. Your card details never touch our servers \u2014 all payment data is processed client-side by the payment checkout directly to the payment provider\'s servers. Privacy: the provider website/privacy.' },
                ...
                { subtitle: 'encrypted file storage (File Storage)', text: 'Generated files (project archives, images, videos) are stored in encrypted file storage with AES-256 server-side encryption. Files are accessed via time-limited signed URLs \u2014 no cookies are set by encrypted file storage.' },
                { subtitle: 'Deployment Platforms (User-Initiated)', text: 'When you deploy projects to Vercel, Netlify, GitHub, or the hosting provider using your stored deploy credentials, those platforms operate under their own cookie and privacy policies. Deployment is entirely user-initiated and optional. Your deploy tokens are encrypted with AES-256-GCM, never stored in plaintext, and never logged.' },
            ]
        },
        {
            id: 'localstorage', icon: Cog, title: '5. Local Storage & Client-Side Data', glow: 'rgba(16,185,129,0.4)', content: [
                { subtitle: 'What Is localStorage', text: 'In addition to cookies, we use browser localStorage to persist user preferences on the client side. localStorage data never leaves your browser unless explicitly synced to our servers. Unlike cookies, localStorage data is not sent with HTTP requests. Data stored on sanbayfusion.com is not readable by spaces.sanbayfusion.com and vice versa (same-origin policy).' },
                { subtitle: 'sanbayfusion.com localStorage Keys', text: 'canvas_dark_mode (dark/light theme preference, persistent), canvas_studio_model (selected AI model name, persistent), canvas_studio_provider (selected AI provider name, persistent), canvas_studio_usage (project/chat counts for UI display, persistent), gencraft_v4_history (project history for quick access, persistent), userEmail (UI display only \u2014 never sent to AI providers, persistent), maulaai_user_id (anonymous visitor UUID for session continuity, persistent).' },
                { subtitle: 'spaces.sanbayfusion.com (Maula Editor) localStorage', text: 'auth_token (JWT for Maula Editor authentication, session-scoped). Additional editor-specific preferences are stored locally and never transmitted to third parties.' },
                { subtitle: 'How to Clear', text: 'Open your browser Developer Tools (F12) \u2192 Application tab \u2192 Local Storage. Select sanbayfusion.com or spaces.sanbayfusion.com and delete individual keys or clear all entries. Alternatively, use your browser\'s privacy/site data settings to clear all data for the domain. Clearing localStorage resets preferences to defaults but does NOT affect server-side account data (projects, chat history, agent memories, billing records remain safe in PostgreSQL and file storage).' },
        ...
        },
        {
            id: 'marketing', icon: Target, title: '7. Marketing & Advertising', glow: 'rgba(236,72,153,0.4)', content: [
                ...
                { subtitle: 'No Third-Party Pixels or Tags', text: 'We do not embed Facebook Pixel, Google Ads tags, Google Tag Manager, LinkedIn Insight, TikTok Pixel, Twitter/X Pixel, Pinterest Tag, Snapchat Pixel, or any other conversion/tracking pixel on sanbayfusion.com or spaces.sanbayfusion.com.' },
        ...
        },
        {
            id: 'managing', icon: Settings, title: '9. Managing Your Cookie Preferences', glow: 'rgba(59,130,246,0.4)', content: [
                ...
                { subtitle: 'Browser Settings \u2014 Chrome', text: 'Settings \u2192 Privacy and Security \u2192 Cookies and other site data. You can block all cookies, block third-party cookies, or clear cookies on exit. You can also manage site-specific cookie permissions for sanbayfusion.com and spaces.sanbayfusion.com.' },
                ...
                { subtitle: 'Mobile Devices', text: 'Safari (iOS): Settings \u2192 Safari \u2192 Website Data. Chrome (Android): Settings \u2192 Privacy \u2192 Clear Browsing Data. Firefox Mobile: Settings \u2192 Privacy \u2192 Manage Data. All mobile browsers support clearing site-specific data for sanbayfusion.com.' },
                ...
                { subtitle: 'Clear localStorage', text: 'Open your browser Developer Tools (F12) \u2192 Application tab \u2192 Local Storage. Select sanbayfusion.com or spaces.sanbayfusion.com and delete individual keys or clear all entries. This resets preferences to defaults but does not affect server-side account data.' },
            ]
        },
        {
            id: 'legal-basis', icon: Fingerprint, title: '10. Legal Basis Summary', glow: 'rgba(99,102,241,0.4)', content: [
                ...
                { subtitle: 'CalOPPA & ePrivacy', text: 'Our cookie practices align with the ePrivacy Directive (2002/58/EC as amended by 2009/136/EC) and CalOPPA requirements. We provide clear information about all cookies and localStorage keys used across sanbayfusion.com and spaces.sanbayfusion.com. Our approach of using only strictly necessary cookies means we comply with even the strictest interpretations of cookie consent requirements.' },
        ...
        },
        {
            id: 'updates', icon: RefreshCw, title: '11. Updates to This Policy', glow: 'rgba(251,146,60,0.4)', content: [
                ...
                { subtitle: 'Your Continued Use', text: 'Your continued use of sanbayfusion.com and spaces.sanbayfusion.com after the updated policy becomes effective constitutes acceptance of the changes, except where consent is required by applicable law including GDPR, PDPA Thailand, PDPA Singapore, and UAE PDPL.' },
            ]
        },
    ];
        ...
        <div ref={containerRef} className="min-h-screen bg-[#030304] text-white overflow-x-hidden">
            ...
            <section className="relative pt-28 pb-16 lg:pt-36 lg:pb-20 z-10">
                <div className="container mx-auto px-4 text-center relative z-10">
                    ...
                    <p className="hero-subtitle text-lg text-gray-400 max-w-2xl mx-auto mb-8 font-light" style={{ opacity: 0 }}>Learn how we use cookies and similar technologies across <span className="text-amber-400">sanbayfusion.com</span> and <span className="text-amber-400">spaces.sanbayfusion.com</span> (Canvas App, Canvas Studio, GenCraft Pro, Maula Editor). <strong className="text-amber-300">We use ZERO third-party tracking cookies and ZERO advertising cookies.</strong></p>
            ...
            <section className="relative py-12 z-10">
                <div className="container mx-auto px-4"><div className="max-w-4xl mx-auto">
                    ...
                    <div className="contact-block mt-14 p-7 rounded-2xl bg-white/[0.02] border border-white/[0.06]">
                        ...
                        <p className="text-gray-600 text-sm mb-6">For questions about our cookie usage, localStorage practices, or how AI services interact with client-side storage across sanbayfusion.com and spaces.sanbayfusion.com, contact our privacy team. We respond within 30 days as required by GDPR, PDPA, and UAE PDPL.</p>
                        <div className="flex flex-wrap gap-3">
                            <Link href="mailto:privacy@sanbayfusion.com" className="px-6 py-3 rounded-xl bg-gradient-to-r from-amber-600/20 to-orange-600/20 border border-amber-500/25 text-amber-400 font-medium text-sm hover:border-amber-500/40 transition-all">privacy@sanbayfusion.com</Link>
                            <Link href="mailto:support@sanbayfusion.com" className="px-6 py-3 rounded-xl bg-gradient-to-r from-cyan-600/20 to-blue-600/20 border border-cyan-500/25 text-cyan-400 font-medium text-sm hover:border-cyan-500/40 transition-all">support@sanbayfusion.com</Link>
                            <Link href="mailto:dpo@sanbayfusion.com" className="px-6 py-3 rounded-xl bg-gradient-to-r from-violet-600/20 to-purple-600/20 border border-violet-500/25 text-violet-400 font-medium text-sm hover:border-violet-500/40 transition-all">dpo@sanbayfusion.com</Link>
