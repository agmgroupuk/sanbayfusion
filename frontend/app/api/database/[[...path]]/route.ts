/**
 * Proxy: /api/database/* → backend /api/database/*
 * Handles database management requests from canvas-app
 */
...
async function proxyRequest(request: NextRequest, params: Promise<{ path?: string[] }>) {
  try {
    ...
    const qs = url.search || '';
    const target = `${BACKEND_URL}/api/database${subPath}${qs}`;
    ...
    console.error('[/api/database] Proxy error:', error);
