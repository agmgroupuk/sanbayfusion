/**
 * Proxy: /api/monitoring/* → backend /api/monitoring/*
 * Handles monitoring/logs/errors/health requests from canvas-app
 */
...
async function proxyRequest(request: NextRequest, params: Promise<{ path?: string[] }>) {
  try {
    ...
    const qs = url.search || '';
    const target = `${BACKEND_URL}/api/monitoring${subPath}${qs}`;
    ...
    console.error('[/api/monitoring] Proxy error:', error);
