/**
 * Proxy: /api/builds/* → backend /api/builds/*
 * Handles build pipeline requests from canvas-app
 */
...
async function proxyRequest(request: NextRequest, params: Promise<{ path?: string[] }>) {
  try {
    ...
    const qs = url.search || '';
    const target = `${BACKEND_URL}/api/builds${subPath}${qs}`;
    ...
    console.error('[/api/builds] Proxy error:', error);
