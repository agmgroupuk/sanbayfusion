...
function LoginPageContent() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#030304' }}>
        <div className="flex flex-col items-center gap-4">
          ...
          <p style={{ color: 'rgba(148,163,184,0.6)' }}>Checking authentication...</p>
        </div>
      </div>
    );
  }
  ...
  if (state.isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#030304' }}>
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 rounded-full animate-spin" style={{ border: '3px solid rgba(147,197,253,0.2)', borderTopColor: 'rgba(186,230,253,0.8)' }} />
          <p style={{ color: 'rgba(148,163,184,0.6)' }}>Already signed in! Redirecting...</p>
        </div>
      </div>
    );
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try { await login(formData.email, formData.password, turnstileToken); }
    catch (error) { console.error('Login error:', error); }
    finally { setIsSubmitting(false); }
  };

  return (
    <div ref={containerRef} className="min-h-screen text-white flex items-center justify-center py-6 px-4 overflow-hidden" style={{ backgroundColor: '#030304' }}>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute bottom-1/3 right-1/3 w-[350px] h-[350px] rounded-full" style={{ background: 'radial-gradient(circle, rgba(186,230,253,0.06) 0%, transparent 70%)' }} />
      ...
      <div className="relative z-10 max-w-md w-full">
        <div>
          ...
          <div className="login-logo text-center py-5 rounded-t-2xl relative overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(147,197,253,0.05) 0%, rgba(186,230,253,0.02) 50%, rgba(96,165,250,0.04) 100%)', border: '1px solid rgba(147,197,253,0.1)', borderBottom: '1px solid rgba(147,197,253,0.06)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.05), 0 4px 24px rgba(0,0,0,0.15)' }}>
            ...
            <h1 className="relative text-xl font-bold leading-tight">
              <span className="login-title" style={{ background: 'linear-gradient(to right, #ffffff, #a5f3fc, #c4b5fd)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Welcome Back</span>
            </h1>
            <p className="login-subtitle relative text-xs mt-1" style={{ color: 'rgba(186,230,253,0.7)' }}>Sign in to access your AI agents</p>
          </div>

          {/* ── Login Form Card ── */}
          <div className="login-form relative p-6 rounded-b-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(147,197,253,0.04) 0%, rgba(186,230,253,0.02) 50%, rgba(96,165,250,0.03) 100%)', border: '1px solid rgba(147,197,253,0.08)', borderTop: 'none', boxShadow: '0 8px 32px rgba(0,0,0,0.4), 0 0 0 1px rgba(147,197,253,0.04), inset 0 1px 0 rgba(186,230,253,0.06)' }}>
            ...
            {/* ── Messages ── */}
            {successMessage && (
              <div className="relative mb-4 p-3 rounded-xl" style={{ background: 'rgba(147,197,253,0.06)', border: '1px solid rgba(147,197,253,0.15)' }}>
                <p className="text-center font-medium text-sm" style={{ color: 'rgba(186,230,253,0.9)' }}>{successMessage}</p>
              </div>
            )}
            {state.error && (
              <div className="relative mb-4 p-3 rounded-xl" style={{ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.2)' }}>
                <p className="text-red-400 text-center font-medium text-sm">{state.error}</p>
              </div>
            )}

            <form onSubmit={handleSubmit} className="relative space-y-5">
              <div>
                <label className="block text-sm font-medium mb-2" style={{ color: 'rgba(186,230,253,0.5)' }}>Email Address</label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium mb-2" style={{ color: 'rgba(186,230,253,0.5)' }}>Password</label>
                <div className="relative">
                  <input
                    type={showPassword ? 'text' : 'password'} name="password" value={formData.password} onChange={handleInputChange} required
                    placeholder="Enter your password"
                    className="w-full px-4 py-3 pr-12 rounded-xl text-white text-sm placeholder-slate-600 focus:outline-none transition-all duration-300"
                    style={{ background: 'rgba(147,197,253,0.04)', border: '1px solid rgba(147,197,253,0.1)', boxShadow: 'inset 0 2px 4px rgba(0,0,0,0.3)' }}
                    onFocus={(e) => { e.currentTarget.style.borderColor = 'rgba(147,197,253,0.25)'; e.currentTarget.style.boxShadow = 'inset 0 2px 4px rgba(0,0,0,0.3), 0 0 12px rgba(147,197,253,0.08)'; }}
                    onBlur={(e) => { e.currentTarget.style.borderColor = 'rgba(147,197,253,0.1)'; e.currentTarget.style.boxShadow = 'inset 0 2px 4px rgba(0,0,0,0.3)'; }}
                  />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3 transition-colors" style={{ color: 'rgba(148,163,184,0.5)' }}>
              ...
              <div className="text-right">
                <Link href="/auth/reset-password" className="text-sm transition-colors duration-200" style={{ color: 'rgba(147,197,253,0.6)' }}>Forgot your password?</Link>
              </div>

              <TurnstileWidget onVerify={setTurnstileToken} onExpire={() => setTurnstileToken(null)} className="mb-1" />

              <button
                type="submit" disabled={isSubmitting || state.isLoading}
                className="w-full py-3 px-4 rounded-xl font-semibold text-white transition-all duration-300 hover:scale-[1.02]"
                style={isSubmitting || state.isLoading
            ...
            <div className="relative flex items-center my-6">
              ...
              <span className="px-4 text-xs font-medium" style={{ color: 'rgba(148,163,184,0.4)' }}>or continue with</span>
              <div className="flex-grow h-px" style={{ background: 'rgba(147,197,253,0.1)' }} />
            </div>
            ...
            <div className="relative grid grid-cols-2 gap-2.5">
              ...
              <a
                href={`/api/auth/microsoft${searchParams.get('redirect') ? `?redirect=${encodeURIComponent(searchParams.get('redirect')!)}` : ''}`}
                className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl text-sm transition-all duration-300 hover:scale-[1.02]"
                style={{ background: 'rgba(147,197,253,0.04)', border: '1px solid rgba(147,197,253,0.1)' }}
                onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'rgba(147,197,253,0.2)'; e.currentTarget.style.background = 'rgba(147,197,253,0.08)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'rgba(147,197,253,0.1)'; e.currentTarget.style.background = 'rgba(147,197,253,0.04)'; }}
              ...
              <a
                href={`/api/auth/yahoo${searchParams.get('redirect') ? `?redirect=${encodeURIComponent(searchParams.get('redirect')!)}` : ''}`}
                className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl text-sm transition-all duration-300 hover:scale-[1.02]"
                style={{ background: 'rgba(147,197,253,0.04)', border: '1px solid rgba(147,197,253,0.1)' }}
                onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'rgba(147,197,253,0.2)'; e.currentTarget.style.background = 'rgba(147,197,253,0.08)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'rgba(147,197,253,0.1)'; e.currentTarget.style.background = 'rgba(147,197,253,0.04)'; }}
        ...
        {/* ── Footer ── */}
        <div className="login-footer text-center space-y-2 mt-4">
          <p style={{ color: 'rgba(148,163,184,0.5)' }}>
            Don&apos;t have an account?{' '}
            <Link href={`/auth/signup${searchParams.get('redirect') ? `?redirect=${searchParams.get('redirect')}` : ''}`} className="font-medium transition-colors duration-200" style={{ color: 'rgba(147,197,253,0.8)' }}>
              Sign up here
            </Link>
          </p>
          <Link href="/home" className="inline-block text-sm transition-colors duration-200" style={{ color: 'rgba(148,163,184,0.4)' }}>← Back to homepage</Link>
        </div>
      </div>
    </div>
  );
