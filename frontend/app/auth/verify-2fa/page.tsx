...
function Verify2FAContent() {
    ...
    useEffect(() => {
        if (state.isAuthenticated) { router.push('/dashboard/overview'); }
    }, [state.isAuthenticated, router]);

    useEffect(() => {
        if (!tempToken || !userId) { router.push('/auth/login'); }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        const verificationCode = useBackupCode ? backupCode.toUpperCase() : code.join('');
        if (!useBackupCode && verificationCode.length !== 6) { setError('Please enter all 6 digits'); return; }
        if (useBackupCode && !backupCode.trim()) { setError('Please enter your backup code'); return; }
        setIsSubmitting(true); setError(null);
        try {
            const response = await fetch('/api/auth/verify-2fa', {
                method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include',
                body: JSON.stringify({ tempToken, userId, code: verificationCode, useBackupCode, turnstileToken }),
            });
            const data = await response.json();
            if (!response.ok) { throw new Error(data.message || 'Verification failed'); }
            if (data.user) { sessionStorage.removeItem('tempToken'); secureAuthStorage.setUser(data.user); window.location.href = '/dashboard/overview'; }
        } catch (err) {
    ...
    useEffect(() => {
        ...
        if (!useBackupCode && verificationCode.length === 6 && !isSubmitting) {
            const form = document.getElementById('verify-form') as HTMLFormElement;
            form?.requestSubmit();
        }
    }, [code, isSubmitting, useBackupCode]);
        ...
        return (
            <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#030304' }}>
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p style={{ color: 'rgba(148,163,184,0.6)' }}>Redirecting to login...</p>
                </div>
            </div>
        );
    }
                ...
                <div>
                    ...
                    <div className="verify-icon text-center py-5 rounded-t-2xl relative overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(147,197,253,0.05) 0%, rgba(186,230,253,0.02) 50%, rgba(96,165,250,0.04) 100%)', border: '1px solid rgba(147,197,253,0.1)', borderBottom: '1px solid rgba(147,197,253,0.06)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.05), 0 4px 24px rgba(0,0,0,0.15)' }}>
                        ...
                        <h1 className="relative text-xl font-bold leading-tight">
                            <span className="verify-title" style={{ background: 'linear-gradient(to right, #ffffff, #a5f3fc, #c4b5fd)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Two-Factor Auth</span>
                        </h1>
                        <p className="verify-subtitle relative text-xs mt-1" style={{ color: 'rgba(186,230,253,0.7)' }}>Enter the 6-digit code from your authenticator app</p>
                    </div>
                    ...
                    <div className="verify-form relative p-8 rounded-b-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(147,197,253,0.04) 0%, rgba(186,230,253,0.02) 50%, rgba(96,165,250,0.03) 100%)', border: '1px solid rgba(147,197,253,0.08)', borderTop: 'none', boxShadow: '0 8px 32px rgba(0,0,0,0.4), 0 0 0 1px rgba(147,197,253,0.04), inset 0 1px 0 rgba(186,230,253,0.06)' }}>
                        ...
                        <form id="verify-form" onSubmit={handleSubmit} className="relative space-y-6">
                            {/* Toggle buttons */}
                            <div className="flex justify-center gap-2 mb-4">
                                <button type="button" onClick={() => { setUseBackupCode(false); setError(null); }}
                                    className="px-4 py-2 rounded-xl text-sm transition-all"
                                    style={!useBackupCode
                                        ? { background: 'rgba(147,197,253,0.08)', color: 'rgba(186,230,253,0.9)', border: '1px solid rgba(147,197,253,0.15)' }
                                        : { color: 'rgba(148,163,184,0.5)', background: 'transparent', border: '1px solid transparent' }
                                    }>
                                    Authenticator Code
                                </button>
                                <button type="button" onClick={() => { setUseBackupCode(true); setError(null); }}
                                    className="px-4 py-2 rounded-xl text-sm transition-all"
                                    style={useBackupCode
                                        ? { background: 'rgba(147,197,253,0.08)', color: 'rgba(186,230,253,0.9)', border: '1px solid rgba(147,197,253,0.15)' }
                                        : { color: 'rgba(148,163,184,0.5)', background: 'transparent', border: '1px solid transparent' }
                            ...
                            {!useBackupCode ? (
                                <div className="flex justify-center gap-3" role="group" aria-label="6-digit verification code">
                                    {code.map((digit, index) => (
                                        <input
                                            ...
                                            onFocus={(e) => { e.currentTarget.style.borderColor = 'rgba(147,197,253,0.3)'; e.currentTarget.style.boxShadow = 'inset 0 2px 4px rgba(0,0,0,0.3), 0 0 16px rgba(147,197,253,0.1)'; }}
                                ...
                                <div className="space-y-3">
                                    <input type="text" value={backupCode}
                                        ...
                                        className="w-full px-4 py-3 text-center text-lg font-mono rounded-xl text-white focus:outline-none transition-all duration-300 tracking-wider uppercase"
                                        style={{ background: 'rgba(147,197,253,0.04)', border: '1px solid rgba(147,197,253,0.1)', boxShadow: 'inset 0 2px 4px rgba(0,0,0,0.3)' }}
                                        ...
                                        disabled={isSubmitting}
                                    />
                                    <p className="text-xs text-center" style={{ color: 'rgba(148,163,184,0.35)' }}>Enter one of your saved backup codes</p>
                                </div>
                            )}
                            ...
                            <button type="submit"
                                disabled={isSubmitting || !turnstileToken || (!useBackupCode && code.join('').length !== 6) || (useBackupCode && !backupCode.trim())}
                                className="w-full py-3 px-4 font-semibold rounded-xl text-white transition-all duration-300 hover:scale-[1.02]"
                                style={isSubmitting || (!useBackupCode && code.join('').length !== 6) || (useBackupCode && !backupCode.trim())
