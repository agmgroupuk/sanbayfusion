...
class RealtimeVoiceService {
  ...
  async connect(config: RealtimeVoiceConfig = {}): Promise<void> {
    try {
      // Get ephemeral token from backend — pass agentId so backend picks correct voice + instructions
      const response = await fetch('/api/realtime/session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ agentId: config.agentId, model: config.model })
      });
  ...
  private async initializeAudio(): Promise<void> {
    ...
    // Buffer size 2048 at 24kHz = ~85ms chunks — smaller = more responsive
    this.processor = this.audioContext.createScriptProcessor(2048, 1, 1);

    this.processor.onaudioprocess = (e) => {
      // Ensure WebSocket is fully OPEN before sending
      if (!this.isConnected || !this.ws || this.ws.readyState !== WebSocket.OPEN) return;      // Don't send audio when muted
   ...
   * When muted, audio is NOT sent to OpenAI — truly silent from agent's perspective
   */
  setMuted(muted: boolean): void {
    this.isMuted = muted;
    // Also stop/start the mic tracks for visual indicator
    if (this.mediaStream) {
