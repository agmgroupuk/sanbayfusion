...
class RealtimeVoiceService {
  ...
  async connect(config: RealtimeVoiceConfig = {}): Promise<void> {
    try {
      ...
      const { realtimeSessionService } = await import('../services/realtimeSessionService');
      const result = await realtimeSessionService.create(config as Record<string, unknown>);

      // Handle both response formats:
      // - Direct backend: { success, session: { client_secret: { value } } }
      // - Next.js proxy:  { client_secret: { value } }
      const secret = result?.client_secret?.value ?? result?.session?.client_secret?.value;
      if (!secret) {
        throw new Error('Failed to get realtime session token — no client_secret in response');
      }

      // Connect to OpenAI Realtime API (latest model for best voice quality)
  ...
  /**
   * Initialize audio capture and playback with premium processing pipeline
   */
  private async initializeAudio(): Promise<void> {
    // Request microphone access with premium audio constraints
    ...
    // Premium output audio chain: source → compressor → gain → destination
    ...
    // Capture audio chunks — buffer size 2048 at 24kHz = ~85ms for lower latency
  ...
  private async playNextChunk(): Promise<void> {
    ...
    // Create source and play through premium audio chain
    const source = this.audioContext.createBufferSource();
    source.buffer = buffer;
    // Route through compressor → gain → destination for consistent volume
