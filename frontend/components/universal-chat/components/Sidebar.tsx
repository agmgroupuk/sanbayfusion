  ...
  const handleAction = async (e: React.MouseEvent, id: string, action: string) => {
    ...
    switch (action) {
      case 'share':
        ...
        { const { toast: t } = await import('./Toast'); t.success('Copied', 'Session ID copied to clipboard.'); }
        break;
      case 'copy':
        navigator.clipboard.writeText(JSON.stringify(session, null, 2));
        { const { toast: t } = await import('./Toast'); t.success('Copied', 'Session log copied to clipboard.'); }
        break;
      case 'download':
